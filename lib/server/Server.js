...
var forwardClientRequestToApi = require("./forwardClientRequestToApi");
...
var Server = {
    ...
    create: function(requestedConfig) {
        ...
        var apiProxy = config.apiProxy || null;
        ...
        if (apiHost || apiProxy) {
            deprecated(
                "Brisket.createServer({ apiHost: 'path/to/api' }) is deprecated. Please use apis config."
            );
        }
        ...
        Object.keys(apis).forEach(function(apiAlias) {
            ...
            brisketEngine.use("/" + apiAlias, forwardClientRequestToApi(apiConfig));
        });
...
function verifyApis(apis) {
    Object.keys(apis).forEach(function(apiAlias) {
        if (typeof apiAlias !== "string") {
            throw new Error(apiAlias + " is not a valid api alias in apis config. It must be a string.");
        }
