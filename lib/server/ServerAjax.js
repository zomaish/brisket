...
var ServerConfigure = {
    ...
    setup: function(apiHost, apiProxy) {
        Backbone.ajax = function(options) {
            var originalUrl = options.url;
            var url = prepend(apiHost, originalUrl);
            var proxy = apiProxy || null;
...
function prepend(apiHost, originalUrl) {
    var urlWithoutLeadingApi = originalUrl.replace(LEADING_API, "");

    return withTrailingSlash(apiHost) + withoutLeadingSlash(urlWithoutLeadingApi);
}
